...
void SendPosition()
{
    SUART_PutStrFl((const unsigned char*)PSTR("AT+CMGS=\""));
    SUART_PutStrFl((const unsigned char*)myTelephone);
    SUART_PutChar('"');
    SUART_PutChar('\r');
    _delay_ms(100);
    if(position.valid)
    {
        ...
        SUART_PutStrFl((const unsigned char*)PSTR("GPS not ready :("));
